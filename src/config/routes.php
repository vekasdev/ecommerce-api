...
use App\controllers\CaptchaCodeController;
...
use App\middlewares\CaptchaCodeMiddleware;
...
$app->group("/api/v1",function(RouteCollectorProxy $app){
    ...
    $app->group("/user",function (RouteCollectorProxy $app){
        $app->post("/sign-in",[UsersController::class,"signIn"])->add(CaptchaCodeMiddleware::class);
    ...
    $app->group("/order",function(RouteCollectorProxy $app){
        ...
        $app->post("/addDiscount/{code:[a-z0-9]+}",[OrdersController::class,"addDiscountCode"]);
    ...
    $app->group("/cart",function (RouteCollectorProxy $app) {
        $app->post("/create-order/{product-id:[0-9]+}/{quantity : [0-9]+}[/{color : [0-9]+}]",[OrdersController::class,"createOrder"]);
    ...
    $app->group("/captcha-code",function(RouteCollectorProxy $app){
        $app->get("/request",[CaptchaCodeController::class,"requestCaptchaCode"]);
    });
    ...
    $app->group("/manage",function(RouteCollectorProxy $app){
        ...
        $app->group("/discount-code",function(RouteCollectorProxy $app){
            ...
            $app->get("/single/{id:[0-9]+}",[DiscountCodeController::class,"getSingleDiscountCode"]);
            $app->delete("/{id:[0-9]+}",[DiscountCodeController::class,"deleteDiscountCode"]);
        });
        $app->group("/order-group",function(RouteCollectorProxy $app){
        ...
        $app->group("/color",function(RouteCollectorProxy $app){
            ...
            $app->post("",[ColorController::class,"createColor"]);
            $app->put("/{id:[0-9]+}",[ColorController::class,"updateColor"]);
            $app->delete("/{id:[0-9]+}",[ColorController::class,"deleteColor"]);
        });

    })->add(AdminUserAuthentication::class);
});

$app->get("/test",function($req,$res,$args) {

    /** @var CaptchaBuilder */
    $captch = $this->get(CaptchaBuilder::class);
    ob_start();
    $captch->output();
    $image = ob_get_clean();
